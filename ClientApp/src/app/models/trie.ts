...
export class Trie{
 ...
 public findAllWord(node: TrieNode, output:any [], word : string = ""): void
 {
          word += node.character;
          if(node.count > 0)
          {
          ...
          {
                 this.findAllWord( node.children.get(key) as TrieNode, output, word);
          }
 }
 ...
 public find(key: string): any[]{
      ...
      this.findAllWord(node, output, key.substring(0, key.length - 1));
      return output;
