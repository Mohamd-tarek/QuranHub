...
@Component({
  selector: "syntax-analysis",
  templateUrl: "syntax-analysis.component.html"
})

export class SyntaxAnalysisComponent implements OnDestroy {
  
  subscription: Subscription;
  currentSyntaxSura :number = 0;
  currentSyntaxAya :number = 0; 
  aya!: Quran;
  ...
  checkLocalStateChange(newState: any) : boolean{
    return ( newState["currentSyntaxAya"]  == this.currentSyntaxAya &&
             newState["currentSyntaxSura"] == this.currentSyntaxSura);
  }

  setState(newState: any):void{
        this.currentSyntaxSura = newState["currentSyntaxSura"];
        this.currentSyntaxAya  = newState["currentSyntaxAya"];
        this.getAya();
  }
  ...
  get curSura(): number {
   return this.currentSyntaxSura;
  }

  set curSura(value : number) {
    this.currentSyntaxSura = value;
    let state: any  = {"currentSyntaxSura" : this.currentSyntaxSura }
    this.stateService.next(state);
    this.curAya = 1;
  }

  get curAya(): number {
    return this.currentSyntaxAya;
   }
 
  set curAya(value : number) {
    this.currentSyntaxAya = value;
    let state: any  = {"currentSyntaxAya": this.currentSyntaxAya}
    this.stateService.next(state);
    this.getAya();
  ...
  getAya(): void{
    this.repo.quran.subscribe((data:any) => { 
      if(data.length > 1){
        this.ayaLoaded = true;
        this.aya = 
          data[this.currentSyntaxSura][this.currentSyntaxAya - 1];
